...
# Create the faulty `compose.yaml`

Outside of the `app` directory create a `compose.yaml`:
...
Run up again and check if it's running.
...
In the `compose.yaml` file, we can use either:
...
Remember to rerun `docker compose up --build` after changing the `compose.yaml` file.
...
# Simplify the `compose.yaml` and let them join the same network
...
There are better solutions that will not require us to push the environment variables but for now let's add them to the compose file:
...
To stop it, you just need to be where the `compose.yaml` file is and run:

```bash
$ docker compose down
```

It will know to stop all the services defined in the `compose.yaml` file.
