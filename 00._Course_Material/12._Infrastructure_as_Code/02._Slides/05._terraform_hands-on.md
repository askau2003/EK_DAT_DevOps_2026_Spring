...
It is missing a configuration file. Create `main.tf`. Then run init again.
...
If you install **GraphViz**, you can visualize the Terraform graph with the dot command:
...
1. Run `terraform apply` and verify that it works.

2. Run `terraform destroy` so that we can move away from passworded SSH access. This requires provisioning a VM from scratch.
...
Using the remote provisioner to install dependencies is not best practices and beyond the purpose of Terraform, but it is quite useful.
...
Since the remote-provisioner only runs when the VM is created, run `destroy` => `apply`. 

For a more elaborate version take a look at [Whoknows Variations](https://github.com/who-knows-inc/whoknows_variations/blob/continuous_deployment/infrastructure/inline_commands.sh).
