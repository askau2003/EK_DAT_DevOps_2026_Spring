...
If only... Then we would be able to run Terraform in CI. For instance, a GitHub Action that uses Terraform to automatically provision a new test environment for every build on a specific branch. That would be **close to pro**!
...
Terraform can hog your computer's resources but the heavy lifting happens on the cloud provider's side. 
