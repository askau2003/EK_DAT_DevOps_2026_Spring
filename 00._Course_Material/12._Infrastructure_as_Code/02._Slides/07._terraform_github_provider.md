...
2. Create a new organisation. This can only be done through the UI or `gh`.
...
See it as a security mechanism when deleting repositories. 

Delete the repository manually in the UI.

Explanation follows in the next slide.
...
1. If it has been set to `true`, then the repository will be archived. Running `tf apply` again from scratch will not work because the repository already exists. The repository must first be manually deleted.
...
[GitHub Apps](https://docs.github.com/en/apps) can help set correct admin permission after provisioning repositories.
