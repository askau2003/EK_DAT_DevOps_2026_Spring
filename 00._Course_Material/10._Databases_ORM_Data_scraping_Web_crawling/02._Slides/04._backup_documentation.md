...
Where the `compose.yaml` file is defined:
...
1. Create a folder `initdb` in the same directory as the `compose.yaml` file.

2. Move the `pgdump.sql` file to the `initdb` folder and rename it to `init.sql`.

3. Load the file as a volume. In `compose.yaml`:
