...
```bash
$ uv run scrapy startproject wikicrawler
$ cd wikicrawler
```
...
```bash
$ uv run scrapy genspider WikiPageSpider https://en.wikipedia.org/wiki/List_of_common_misconceptions
```
...
```bash
$ uv run scrapy crawl WikiPageSpider -o output.json
```
...
```bash
$ uv run scrapy shell https://en.wikipedia.org/wiki/List_of_common_misconceptions
```
