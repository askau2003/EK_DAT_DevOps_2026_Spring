...
```powershell
$ scoop install kubernetes-cli minikube
```
...
You have to specify the driver.
...
# Exposing the deployment to access a pod
...
# Exposing the deployment using minikube
...
Let's create a deployment for a simple Express server running on port `8080` (image in my account):

```bash
$ kubectl create deployment k8s-server --image=andlocker/k8s-server
$ kubectl expose deployment k8s-server --port=8080 --type=NodePort
```

Get the URL:

```bash
$ minikube service k8s-server
```
...
```bash
$ kubectl scale deployment k8s-server --replicas=4
```

Create the TCP / port-forwarding tunnel again and refresh to check out the different pods:

```bash
$ minikube service k8s-server
```

**Note**: Hit hard refresh to avoid caching.
...
| `name` (x2)     | `k8s-server`                 |
| `app` (x2)     | `k8s-server`                 |
| `image`         | `andlocker/k8s-server:latest`|
| `containerPort` | `8080`                            |

The containerPort is `8080` because my Express server is running on port `8080`.
...
The `targetPort` in the service must be the same as the `containerPort` in the deployment.
