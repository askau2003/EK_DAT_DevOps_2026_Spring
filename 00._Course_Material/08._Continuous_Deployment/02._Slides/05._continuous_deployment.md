...
5. `compose.yaml` file is on the server -> `docker compose pull` / `docker compose up`
...
2. `scp` the `compose.yaml` file onto the server -> `docker compose pull` -> `docker compose up`
...
1. The first option would not work if there are changes to the `compose.yaml` file. It requires a manual step to place the file on the server.
...
**Campfire** and **Facebook GateKeeper** have just been mentioned. 
